...
public class DetalleFactura {
    ...
    public static void Show() {
       ...
       for (int i = 0; i <menus.facturas.size(); i++) {
        menus.ListaClientes.get(i).Show();
       }
        ...
        for (int i = 0; i < menus.pedido.size(); i++) {
            ...
            value += (String.format("%-10s", (i + 0001) +"    " +
            "     " + (String.format("%-15s", menus.pedido.get(i).getNombre()) 
        +   "  " + (String.format("%-12s", String.valueOf( menus.pedido.get(i).getPrecio()) +
            "         " +(String.format("%-12s", String.valueOf( menus.pedido.get(i).getCantidad()) 
        +   "        " + (String.format("%-10s\n",menus.pedido.get(i).getTotal())))))))));
        }


        System.out.println("TOTAL DE LA FACTURA=>>" + menus.totalF);
